...
frappe.listview_settings['Hbs Crm Lead'] = {
	...
	formatters: {
		company_name(val, df, doc) {
			...
			return `
				<span style="display: inline-flex; align-items: center; justify-content: space-between; gap: 6px; width: 100%; min-width: 180px;">
					<span class="text-truncate" style="max-width: calc(100% - 26px); display: inline-block;" title="${text}"><b>${text}</b></span>
					<span style="cursor: help; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center; width: 20px; height: 20px; border-radius: 50%; background: #eff6ff; color: #2563eb; border: 1px solid #bfdbfe;" title="Latest Remark:&#10;${remark}">
